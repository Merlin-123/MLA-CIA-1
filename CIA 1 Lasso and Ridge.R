...
# Convert the State variable to a factor
hr.data.1$Department <- as.factor(hr.data.1$Department)
hr.data.1$Education.Field <- as.factor(hr.data.1$Education.Field)
hr.data.1$Job.Role <- as.factor(hr.data.1$Job.Role)
hr.data.1$Education <- as.factor(hr.data.1$Education)
hr.data.1$Over.Time <- as.factor(hr.data.1$Over.Time)

# Create model matrix (including dummy variables for State)
X <- model.matrix(Monthly.Income ~ ., hr.data.1)[, -1]  # Exclude the intercept term

# Separate the target variable
Y <- hr.data.1$Monthly.Income
